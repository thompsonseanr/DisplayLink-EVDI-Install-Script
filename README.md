...
| **openSUSE Tumbleweed** | May have to run: `systemctl restart displaylink-driver.service` after login or manually unplug and re-plugin the docking station connection (I experienced this in `Arch` years ago and ran a script that was essentially a service restart hook). The solution is most likely related to the `usb` device instantiation and the solution out of scope of this script. Tried blacklisting `udl` and `udlfb` drivers to no avail. Reference: [HowTo build Displaylink driver for OpenSuse Tumbleweed with latest kernel](https://0xcaffee.blog/posts/opensuse-tumbleweed-evdi/). For research purposes, the `udev` rule is: `/usr/lib/udev/rules.d/99-displaylink.rules`. Reference: [systemctl enable displaylink.service](https://github.com/displaylink-rpm/displaylink-rpm/issues/50) |
